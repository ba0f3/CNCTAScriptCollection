...
function initOptions(){
    ...
    if (bs) bas.add(opBtn, { left: null, right: 66, bottom: 80 });
     ...
     leftBtn.addListener("click",function(){
                                    ...
                                    /////Tools Button//////
                                    bas.remove(toolsBtn);
                                    toolsBtn.set({width:76});
                                    bas.add(toolsBtn, { right: null, left: 6, bottom: 42 });
                                    /////Simulate Button//////
                                    bas.remove(simBtn);
                                    simBtn.set({width:76});
                                    bas.add(simBtn, { right: null, left: 6, bottom: 4 });
                                    /////Shift Button//////
                                    bas.remove(shiftBtn);
                                    shiftBtn.set({width:76});
                                    bas.add(shiftBtn, { right: null, left: 6, bottom: 118 });
     ...
     rightBtn.addListener("click",function(){
                                    ...
                                    bas.remove(toolsBtn);
                                    toolsBtn.set({width:30});
                                    bas.add(toolsBtn, { right: 66, left: null, bottom: 42 });
         
                                    bas.remove(simBtn);
                                    simBtn.set({width:30});
                                    bas.add(simBtn, { right: 66, left: null, bottom: 4 });

                                    bas.remove(shiftBtn);
                                    shiftBtn.set({width:30});
                                    bas.add(shiftBtn, { right: 66, left: null, bottom: 118 }); 
...
function initSimulateBattle(){
	...
	var application = qx.core.Init.getApplication();
	var bas = application.getUIItem(ClientLib.Data.Missions.PATH.BAR_ATTACKSETUP);
	simBtn = new qx.ui.form.Button("S");
    var bs = stob(localStorage.getItem('btnSide'));
    
	if (bs) simBtn.set({ width: 30, 
                 height: 30,
                 toolTipText: "Simulates Combat"   
               });
    else simBtn.set({ width: 76, 
                 height: 30,
                 toolTipText: "Simulates Combat"   
               });
    
	simBtn.addListener("click", function(){
		if (lock)                 
            return;
        ...
        if (bs) bas.add(simBtn, { left:null, right: 66, bottom: 4 });
        else bas.add(simBtn, { right: null, left: 6, bottom: 4 });
}
...
function initTools(){
    ...
    //////////End Labels//////////////
     toolsBtn = new qx.ui.form.Button("T");
     if (bs) toolsBtn.set({
                width: 30, height: 30,
                appearance: "button-text-small",
    ...
    if (bs) armyBar.add(toolsBtn, {bottom: 42, right: 66, left: null});
    else armyBar.add(toolsBtn, {bottom: 42, right: null, left: 6});
     var tabView = new qx.ui.tabview.TabView();
     tabView.setPadding(5);
...
function initFormationShiftKeys(){
	...
	var bas = application.getUIItem(ClientLib.Data.Missions.PATH.BAR_ATTACKSETUP);
    var bs = stob(localStorage.getItem('btnSide'));
    shiftBtn = new qx.ui.form.Button("F");
    if (bs) shiftBtn.set({ width: 30, 
               height: 30,
               toolTipText: "Shift Formation"
                });
    else shiftBtn.set({ width: 76, 
               height: 30,
               toolTipText: "Shift Formation"
                });
    ...
    if (bs) bas.add(shiftBtn, { left: null, right: 66, bottom: 118 }); 
    else bas.add(shiftBtn, { right: null, left: 6, bottom: 118 }); 
