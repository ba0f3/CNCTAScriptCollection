...
function initializeTools() {
	...
	toolsbtn.addListener("click", function () {
		...
		toolsbtn.setOpacity(0.1);
        setTimeout(function () {
            toolsbtn.setOpacity(1.0);
		}, 10100)
			...
			if (loadBase())
			{
				...
				calcTroops(battleground);
				calcResources();
				
				//labels.damage.structures.overall.setValue(city.GetBuildingsConditionInPercent());
				//labels.damage.overall.setValue(city.GetDefenseConditionInPercent());
	...
	t.add(new qx.ui.basic.Label("Enemy Base:"), {
		...
		row: 0,
		column: 1
		}), t.add(new qx.ui.basic.Label("Defences (Overall):"), {
		row: 1,
		column: 0
		}), labels.damage.units.overall = new qx.ui.basic.Label(" - "), t.add(labels.damage.units.overall, {
		row: 1,
		column: 1
		}), t.add(new qx.ui.basic.Label("Defences (Infantry):"), {
		row: 2,
		column: 0
		}), labels.damage.units.infantry = new qx.ui.basic.Label(" - "), t.add(labels.damage.units.infantry, {
		row: 2,
		column: 1
		}), t.add(new qx.ui.basic.Label("Defences (Vehicle):"), {
		row: 3,
		column: 0
		}), labels.damage.units.vehicle = new qx.ui.basic.Label(" - "), t.add(labels.damage.units.vehicle, {
		row: 3,
		column: 1
		}), t.add(new qx.ui.basic.Label("Defences (Structure):"), {
		row: 4,
		column: 0
		}), labels.damage.units.structure = new qx.ui.basic.Label(" - "), t.add(labels.damage.units.structure, {
		row: 4,
		column: 1
...
function calcResources() {
	try {          
		...
		/*
			Display.lootArray = [];
			Display.lootArray[0] = loots[6];//imgResearch 6 
			Display.lootArray[1] = loots[1];//imgTiberium 1
			Display.lootArray[2] = loots[2];//imgCrystal 2
			Display.lootArray[3] = loots[3];//imgCredits 3 
			store('lootArray',Display.lootArray);
		*/
...
function calcTroops(battleground) {
	try {
		...
		for (var j in el.Defences) {
			var unit = el.Defences[j];
			var current_hp = unit[Data.Bypass.keys.Hitpoints]();
			troops[0] += current_hp;
			if (true) {
				switch (unit[Data.Bypass.keys.Type].at) {
					case ClientLib.Base.EUnitType.Infantry:
					troops[1] += current_hp;
					break;
					case ClientLib.Base.EUnitType.Tank:
					troops[2] += current_hp;
					break;
					case ClientLib.Base.EUnitType.Air:
					troops[3] += current_hp;
					break;
					case ClientLib.Base.EUnitType.Structure:
					troops[4] += current_hp;
					break;
				}
			}
		...
		labels.damage.units.overall.setValue(troops[0]);
		labels.damage.units.infantry.setValue(troops[1]);
		labels.damage.units.vehicle.setValue(troops[2]);
		labels.damage.units.structure.setValue(troops[4]);
		
		
		var units = getCityPreArmyUnits().get_ArmyUnits().l;
		// Get MaxHealth for UnitTypes
                var totalInfantryHealth = 0;
                var InfantryHealth = 0;
                var totalVehicleHealth = 0;
                var totalAirHealth = 0;
                for (var i in units) {
                    switch (units[i].get_UnitGameData_Obj().at) {
						case ClientLib.Data.EUnitGroup.Infantry:
                        InfantryHealth += (units[i].get_Health());
                        totalInfantryHealth += (units[i].get_MaxHealth());
                        break;
						case ClientLib.Data.EUnitGroup.Vehicle:
                        totalVehicleHealth += (units[i].get_MaxHealth());
                        break;
						case ClientLib.Data.EUnitGroup.Aircraft:
                        totalAirHealth += (units[i].get_MaxHealth());
                        break;
						default:
                        alert("CombatSim: Unknown UnitType found!");
                        break;
		...
		labels.health.overall.setValue(totalInfantryHealth+totalVehicleHealth+totalAirHealth);
		labels.health.infantry.setValue(totalInfantryHealth);
		labels.health.vehicle.setValue(totalVehicleHealth);
		labels.health.aircraft.setValue(totalAirHealth);
		
		setTimeout(function() {
			labels.time.setValue(battleground.get_BattleDuration()/1000);
		}, 1000);
		/*
			stats.mnmn = e.get_Simulation().get_iCombatStep() * e.get_TimePerStep() / 1000;
			...
			stats.repair.overall = Math.max(stats.repair.vehicle;
			stats.repair.aircraft;
			stats.repair.infantry);
		*/
		/*
			Display.troopsArray = troops;
			store('troopsArray',Display.troopsArray);
		*/
		} catch (e) {
