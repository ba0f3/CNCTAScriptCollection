				...
				members: {
					...
					activate: function () {
						if (!this.activated) {
							...
							this.SearchFilter();
							qx.locale.Manager.getInstance().addTranslation("en_US", { "tnf:select all": "Select All" });
							qx.locale.Manager.getInstance().addTranslation("en_US", { "tnf:select none": "Select None" });
							qx.locale.Manager.getInstance().addTranslation("en_US", { "tnf:cannot manually modify": "Cannot be modified with multiple rows selected" });
							qx.locale.Manager.getInstance().addTranslation("en_US", { "tnf:trading with multiple bases": "Trading with multiple bases" });
							qx.locale.Manager.getInstance().addTranslation("en_US", { "tnf:percent buttons": "Please use one of the Percent buttons" });
							this.activated = true;
						};
					},
					deactivate: function () {
					...
					tradeWindow: function () {
						...
						searchButton.addListener("execute", this.SearchFilter, this);
					...
					SearchFilter: function () {
						...
						if (currentCity != null) {
							...
							for (var cm in allCities.d) {
								if (currentCity.get_Id() != cm && allCities.d[cm].IsOwnBase()) {
									var otherCity = allCities.d[cm];
									var cs = cm;
									var otherCityName = otherCity.get_Name();
									var cj = ClientLib.Base.Util.CalculateDistance(currentCity.get_X(), currentCity.get_Y(), otherCity.get_X(), otherCity.get_Y());
									var cn = currentCity.CalculateTradeCostToCoord(otherCity.get_X(), otherCity.get_Y(), 1000);
									var co = Math.floor(otherCity.GetResourceCount(this.resourceTransferType));
									var ck = Math.floor(otherCity.GetResourceMaxStorage(this.resourceTransferType));
									var cq = false;
									var ch = false;
									if (this.userDefinedMaxDistance != -1 && this.userDefinedMaxDistance < cj)
										cq = true;
									if (this.userDefinedMinimumAmount != -1 && this.userDefinedMinimumAmount > co)
										ch = true;
									if (!cq && !ch) {
										var cp = phe.cnc.gui.util.Numbers.formatNumbers(co);
										ct.push({
											Base: otherCityName,
											Distance: cj,
											Credits: cn,
							...
							this.tableColumnModel.setDataAsMapArray(ct, true);
							this.selectedRow = null;
							this.selectedRowData = null;
							this.tradeWindowTable.resetCellFocus();
					...
					ChangeResourceType: function (e) {
						...
						this.SetCostLabel();
						this.tradeWindowTable.resetSelection();
						this.resourceTransferType = bN.getUserData("key");
					...
					CalculateTradeCost: function () {
						...
						if (this.transferWindowTableSelectedRows.length > 0) {
							...
							if (this.transferWindowTableSelectedRows.length > 1) {
								...
								this.totalTransferAmount += cities[this.transferWindowTableSelectedRows[0].ID].CalculateTradeCostToCoord(selectedCity.get_PosX(), selectedCity.get_PosY(), parseInt(this.transferAmountTextField.getValue(), 10) * this.modifier);
							}
					...
					TenPercent: function () {
						...
						this.transferAmountTextField.setValue(resourceAmount.toString());
						this.SetCostLabel();
					},
							...
							if (currentCity != null && this.transferAmountTextField.getValue() != "") {
								for (var base in this.transferWindowTableSelectedRows) {
									...
									if (currentBase != null && currentBase.CanTrade() == ClientLib.Data.ETradeError.None && currentCity.CanTrade() == ClientLib.Data.ETradeError.None) {
										...
										if (this.transferWindowTableSelectedRows.length == 1) {
											var transferAmount = parseInt(this.transferAmountTextField.getValue(), 10);
										} else {
											var transferAmount = parseInt(this.transferWindowTableSelectedRows[base].Amount * this.modifier, 10);
										}
					...
					UpdateTradeTableData: function () {
						...
						if (currentCity != null) {
							...
							for (var row in transferWindowsTableData) {
								var otherCity = ClientLib.Data.MainData.GetInstance().get_Cities().GetCity(transferWindowsTableData[row].ID);
							...
							if (this.selectedRow != null) {
								...
								var otherCity = ClientLib.Data.MainData.GetInstance().get_Cities().GetCity(selectedRowData.ID);
					...
					MaintainTradeWindow: function () {
						...
						if (this.transferWindowTableSelectedRows.length > 0) {
							...
							var resourcesInTextField = parseInt(this.transferAmountTextField.getValue(), 10);
							var tradeCost = this.CalculateTradeCost();
							...
							if (playerCreditCount < tradeCost) {
								...
								this.costToTradeLabel.setTextColor("text-label");
							};
						...
						this.transferAmountTextField.setReadOnly(this.tradeWindowTable.getSelectionModel().getSelectedCount() > 1);
